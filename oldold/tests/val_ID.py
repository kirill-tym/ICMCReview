...
def main():
    ...
    tokenizer_id = "./sft_checkpoints/checkpoint-20"
    ...
    # Путь к ID датасету (подправь название файла, если оно отличается)
    id_dataset_path = "../old/data/test_id.jsonl"

    with open(id_dataset_path, "r") as f:
        id_data = [json.loads(line) for line in f]

    success_count = 0
    total = len(id_data)

    print(f"Evaluating {total} ID examples from {id_dataset_path}...")
    ...
    print(f"\nFinal ID Semantic Success Rate: {(success_count / total) * 100:.2f}%")
