...
# --- Функции наград (Reward Functions) ---
def format_reward_func(completions, **kwargs):
    """Награда за строгое соблюдение XML-структуры."""
    rewards = []
...
def numbers_usage_reward_func(completions, target_numbers, **kwargs):
    """Награда за использование ровно тех чисел, что были в условии."""
    rewards = []
...
def math_reward_func(completions, target_numbers, **kwargs):
    """
    Outcome-based reward (ICML 2025):
    Строгая награда +1.0 выдается ТОЛЬКО за корректное выражение, равное 24.
    """
    rewards = []
    for c, target in zip(completions, target_numbers):
        expr = extract_answer(c)
...
def main():
    # Путь к нашему SFT-чекпоинту
    model_id = "./sft_format_teacher/final"
    dataset_path = "../data/train_id.jsonl"
    output_dir = "../grpo_game24_v2"
    ...
    training_args = GRPOConfig(
        ...
        learning_rate=2e-6,  # Низкий LR для стабильности RL
        logging_steps=5,
        per_device_train_batch_size=1,
        gradient_accumulation_steps=8,  # Эффективный батч = 8
        num_generations=8,  # Количество сэмплируемых гипотез на один промпт
        max_completion_length=512,
        temperature=0.7,  # Оптимальная температура для сэмплирования траекторий
        beta=0.0,  # Отключаем KL-штраф для экономии VRAM/RAM
        save_strategy="steps",
        save_steps=100,
        use_vllm=False,
        report_to="none"
    ...
    print("Инициализация GRPOTrainer...")
    trainer = GRPOTrainer(
        model=model,
